...
""" Code for the robot localization project """
...
class ParticleFilter(Node):
    ...
    def __init__(self):
        ...
        self.xy_scale = 0.1             # scale the xy normal distribution
        self.theta_scale = 0.005        # scale the angle normal distribution
    ...
    def resample_particles(self):
        ...
        for i in range(0, self.n_particles):
            samples[i].x = float(np.random.normal(samples[i].x, self.xy_scale, 1)) # 1/samples[i].w, 1))
            samples[i].y = float(np.random.normal(samples[i].y, self.xy_scale, 1)) #1/samples[i].w, 1))
            samples[i].theta = float((np.random.normal(samples[i].theta, self.theta_scale, 1)))
    ...
    def initialize_particle_cloud(self, timestamp, xy_theta=None):
        ...
        x_positions = np.random.normal(xy_theta[0], self.xy_scale, self.n_particles)
        y_positions = np.random.normal(xy_theta[1], self.xy_scale, self.n_particles)
        thetas = np.random.normal(xy_theta[2], self.theta_scale, self.n_particles) 
    ...
    def normalize_particles(self):
        ...
        if sum_weights != 0.0:
            for p in self.particle_cloud:
                p.w /= sum_weights
        else: # this case handles /0.0 errors
            for p in self.particle_cloud:
                p.w = 0.0
    ...
    def publish_particles(self, timestamp):
        ...
        # send the message so that we can view it in rviz
    ...
    def scan_received(self, msg):
        ...
        # Throw away scans until we are done processing the previous scan
